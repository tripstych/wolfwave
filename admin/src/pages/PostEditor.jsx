...
export default function PostEditor() {
  ...
  const handleMediaSelect = (media) => {
    if (mediaPickerTarget === 'og_image') {
      ...
      if (parts.length === 1) {
        ...
        const newRepeaterValue = [...(post.content[regionName] || [])];
  ...
  const handleAiGenerate = async () => {
    if (!post.template_id) return toast.error('Select a template first');
    const prompt = window.prompt('What content should we generate?', post.title);
    if (!prompt) return;

    setGenerating(true);
    const toastId = toast.loading('Generating content, please wait...');
    try {
      const response = await api.post('/ai/generate-content', { templateId: post.template_id, prompt });
      if (response.success && response.data) {
        const textContent = {};
        const newAiPrompts = {};
        ...
        setPost(p => ({ ...p, content: { ...p.content, ...textContent } }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    {regions.map((region) => (
                      <div key={region.name}>
                        ...
                        {region.type === 'repeater' ? (
                          <RepeaterField
                            region={region}
                            value={post.content[region.name]}
        ...
        <div className="space-y-6">
          ...
          <div className="card p-6 space-y-4">
            <div>
              ...
              <button
                ...
                disabled={generating || !post.template_id}
          ...
          <SEOSection
            data={post}
            onChange={(newData) => setPost(newData)}
            openMediaPicker={openMediaPicker}
          />
        </div>
      </div>

      {post.id && <ContentGroupsWidget contentId={post.content_id || post.id} />}
