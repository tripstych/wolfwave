# Plan: Assisted Import - All-in-One AI-Powered Site & Theme Importer
...
## Core Mandates
1. **Separation:** Completely independent of the legacy manual importer.
...
### 2. Core Engines (`server/services/assisted-import/`)
...
- **AssistedImportService**: Orchestrator that runs the multi-phase process.

### 3. API (`server/api/assisted-import.js`)
- `POST /api/assisted-import` - Start an import.
- `GET /api/assisted-import/sites/:id` - Monitor status and ruleset.
- `POST /api/assisted-import/sites/:id/generate-rules` - Manual rule refresh.
...
Trigger via API:
```bash
curl -X POST http://localhost:3000/api/assisted-import -d '{"url": "https://example.com"}'
```
