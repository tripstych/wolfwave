...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    // Fetch content for these posts manually
    ...
    const transformedPosts = posts.map(post => {
      const postContent = contentMap[post.content_id];
      try {
        return {
          ...post,
        ...
        return {
          ...
          content: {},
          title: postContent?.title || post.title,
          slug: postContent?.slug || post.slug
        };
      }
    });
...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    ...
    if (post.content_id) {
      postContent = await prisma.content.findUnique({
...
router.put('/:id', requireAuth, requireEditor, async (req, res) => {
  try {
    ...
    if (og_image && og_image.startsWith('http')) {
      processedOgImage = await downloadMedia(og_image, title || existingPost.title, userId);
    }
    ...
    const processContentMedia = async (obj) => {
      ...
      for (const [key, val] of Object.entries(newObj)) {
        if (typeof val === 'string') {
          if (val.startsWith('http') && (val.match(/\.(jpg|jpeg|png|webp|gif|svg|mp4|webm|ogg|mov)/i))) {
            newObj[key] = await downloadMedia(val, title || existingPost.title, userId);
          } else if (val.includes('<img') || val.includes('<video')) {
            newObj[key] = await processHtmlMedia(val, userId);
          }
        } else if (typeof val === 'object') {
    ...
    if (content || title || slug) {
      if (existingPost.content_id) {
        ...
        const contentUpdates = {
          ...
          ...(title && { title }),
          ...(slug && { slug }),
          search_index: generateSearchIndex(title || existingPost.title, processedContent)
        };
...
router.post('/:id/duplicate', requireAuth, requireEditor, async (req, res) => {
  try {
    ...
    // Fetch content manually
    const originalContent = originalPost.content_id 
      ? await prisma.content.findUnique({ where: { id: originalPost.content_id } })
      : null;
    ...
    const newContent = await prisma.content.create({
      data: {
        module: 'posts',
        title: `${originalContent?.title || originalPost.title} (Copy)`,
        slug: newSlug,
