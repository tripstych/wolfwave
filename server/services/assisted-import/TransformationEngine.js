...
export class TransformationEngine {
  ...
  async run() {
    try {
      info(this.dbName, 'ASSISTED_IMPORT_TRANSFORM_START', `Starting transformation for site ${this.siteId}`);
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (i % 10 === 0) {
          await AssistedImportService.updateStatus(this.siteId, 'transforming', `Migrating items to CMS ${i + 1}/${items.length}...`);
        }

        info(this.dbName, 'ASSISTED_IMPORT_TRANSFORM_ITEM', `Transforming ${item.url} using ${groupRules.page_type} selectors`);
      ...
      await AssistedImportService.updateStatus(this.siteId, 'completed', 'Transformation and Media sideloading complete!');
      info(this.dbName, 'ASSISTED_IMPORT_TRANSFORM_COMPLETE', `Transformation finished for ${items.length} items`);

    } catch (err) {
      logError(this.dbName, err, 'ASSISTED_IMPORT_TRANSFORM_FAILED');
      throw err;
    }
  }
