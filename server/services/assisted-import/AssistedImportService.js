...
export class AssistedImportService {
  ...
  static async nukeSiteData(dbName) {
    info(dbName, 'ASSISTED_IMPORT_NUKE_START', 'Nuking existing site data for fresh import');
    ...
    info(dbName, 'ASSISTED_IMPORT_NUKE_COMPLETE', 'Site data cleared');
  }
  ...
  static async startImport(siteId, rootUrl) {
    ...
    // Run in background
    AssistedImportService._runFullProcess(siteId, rootUrl, dbName).catch(err => {
      logError(dbName, err, 'ASSISTED_IMPORT_CRITICAL');
    });
  ...
  static async _runFullProcess(siteId, rootUrl, dbName) {
    ...
    await runWithTenant(dbName, async () => {
      try {
        ...
        if (config.nuke) {
          await AssistedImportService.updateStatus(siteId, 'nuking', 'Clearing existing site data...');
          await AssistedImportService.nukeSiteData(dbName);
        }

        // Check for cancellation before each major phase
        if (jobRegistry.isCancelled(siteId)) throw new Error('IMPORT_CANCELLED');

        await AssistedImportService.updateStatus(siteId, 'analyzing', 'Starting site discovery...');
        ...
        try {
          await AssistedImportService.updateStatus(siteId, 'analyzing', 'Sideloading theme assets...');
          ...
          logError(dbName, assetErr, 'ASSISTED_IMPORT_ASSET_SIDELOAD_FAILED');
          await AssistedImportService.updateStatus(siteId, 'analyzing', 'Asset sideloading failed, continuing with crawl...');
        }

        if (jobRegistry.isCancelled(siteId)) throw new Error('IMPORT_CANCELLED');
        await AssistedImportService.updateStatus(siteId, 'crawling', 'Starting site crawl...');
        ...
        if (jobRegistry.isCancelled(siteId)) throw new Error('IMPORT_CANCELLED');
        await AssistedImportService.updateStatus(siteId, 'generating_rules', 'Analyzing page structures...');
        ...
        if (jobRegistry.isCancelled(siteId)) throw new Error('IMPORT_CANCELLED');
        await AssistedImportService.updateStatus(siteId, 'generating_templates', 'Generating Nunjucks templates...');

        // Phase 4: Template Generation
        const templateGen = new TemplateGenerator(siteId, dbName);
        await templateGen.run();

        info(dbName, 'ASSISTED_IMPORT_READY', `Import process for ${rootUrl} is ready for manual finalization`);
        await AssistedImportService.updateStatus(siteId, 'ready', 'Templates generated. Ready for final migration!');

      } catch (err) {
        if (err.message === 'IMPORT_CANCELLED') {
          info(dbName, 'ASSISTED_IMPORT_CANCELLED', `Import for ${rootUrl} was cancelled/stopped.`);
          ...
          logError(dbName, err, 'ASSISTED_IMPORT_PROCESS_FAILED');
          try {
            await AssistedImportService.updateStatus(siteId, 'failed', `Error: ${err.message}`);
          } catch {}
        }
      } finally {
